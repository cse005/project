...
from .views.shared import is_otp_valid
...
class KisanAsaraTests(TestCase):
    ...
    def _farmer_registration_payload(self):
        return {
            'name': 'Ramesh Kumar',
            'mobile': '9123456789',
            'age': '30',
            'gender': 'Male',
            'passbook': 'T12345678901',
            'pincode': '503224',
            'state': 'Telangana',
            'district': 'Nizamabad',
            'mandal': 'Armoor',
    ...
    def _tractor_registration_payload(self):
        return {
            'name': 'Tractor Driver',
            'mobile': '9234567890',
            'age': '28',
            ...
            'mandal': 'Armoor',
            'village': 'Perkit',
            'license_id': 'TS0520230000123',
            'services': ['Ploughing'],
            'exp_Ploughing': '5',
            'wage_Ploughing': '600',
        }

    def _labor_registration_payload(self):
        return {
            'name': 'Labor Person',
            'mobile': '9345678901',
            'age': '25',
    ...
    def test_farmer_registration_post_sets_otp(self):
        response = self.client.post(reverse('farmer_register'), self._farmer_registration_payload())
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('verify_otp'))
        self.assertTrue(self.client.session.get('reg_otp'))
        self.assertEqual(self.client.session.get('reg_otp_back_url'), reverse('farmer_register'))

    def test_farmer_registration_invalid_name_blocked(self):
        payload = self._farmer_registration_payload()
        payload['name'] = 'Ramesh123'
    ...
    def test_logout_clears_farmer_session(self):
        self._set_session(self.farmer, 'farmer')
    ...
    def test_main_home_loads(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Farmer's Portal")

    def test_all_registration_posts_redirect_to_verify_otp(self):
        cases = [
            ('tractor_register', self._tractor_registration_payload()),
            ('labor_register', self._labor_registration_payload()),
        ]
    ...
    def test_role_guard_blocks_wrong_provider_actions(self):
        self._set_session(self.labor_user, 'labor')
        response = self.client.get(reverse('dashboard', kwargs={'role': 'tractor'}))
        self.assertRedirects(response, reverse('login'))

    def test_provider_dashboards_include_cancelled_filter(self):
        cases = [
            (self.labor_user, 'labor'),
            (self.tractor_user, 'tractor'),
            (self.tools_user, 'tools'),
    ...
    def test_registration_otp_expires_after_five_minutes(self):
        payload = self._farmer_registration_payload()
        self.client.post(reverse('farmer_register'), payload)
        session = self.client.session
        session['reg_otp'] = {
            'code': '1234',
            'expires_at': (timezone.now() - timedelta(seconds=1)).isoformat(),
        }
        session['reg_otp_back_url'] = reverse('farmer_register')
        session.save()

        response = self.client.post(reverse('verify_otp'), {'otp': '1234'})
        self.assertRedirects(response, reverse('farmer_register'))
        self.assertFalse(self.client.session.get('reg_otp'))

    def test_registration_otp_attempt_limit_blocks_on_fifth_failure(self):
        payload = self._farmer_registration_payload()
        self.client.post(reverse('farmer_register'), payload)
        session = self.client.session
        session['reg_otp'] = {
        ...
        response = None
        for _ in range(5):
            response = self.client.post(reverse('verify_otp'), {'otp': '9999'})

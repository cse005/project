...
def _clean_location_part(value):
    ...
    if cleaned.lower() == 'pickup from shop':
        return ''
    return cleaned


def _compose_location_query(*parts):
    return ', '.join(part for part in (_clean_location_part(item) for item in parts) if part)


def _build_google_maps_url(*parts):
    query = _compose_location_query(*parts)
    if not query:
        return ''
    return f"https://www.google.com/maps/search/?api=1&query={quote_plus(query)}"


def _extract_hourly_rates(raw_tools_type):
    tool_prices = {}
    if not raw_tools_type:
        return tool_prices

    for item in raw_tools_type.split('|'):
        match = re.search(r'([A-Za-z]+)\s*\((?:Rs\.?|₹)\s*(\d+)\s*/\s*hr\)', item.strip(), re.IGNORECASE)
        if not match:
            continue
        tool_prices[match.group(1).strip()] = int(match.group(2))
    return tool_prices
...
def dashboard(request, role):
    if not check_login(request):
        return redirect('login')
    if not _ensure_role(request, role):
...
def farmer_booking(request):
    ...
    farmer = get_logged_in_user(request)
    service_filter = (request.GET.get('service') or 'all').strip().lower()
    ...
    if status_filter != 'all':
        status_value = status_filter.title()
        labor_bookings = labor_bookings.filter(status=status_value)
        tractor_bookings = tractor_bookings.filter(status=status_value)
        tool_bookings = tool_bookings.filter(status=status_value)
        lease_requests = lease_requests.filter(status=status_value)
        shop_orders = shop_orders.filter(status=status_value)

    tractor_bookings = list(tractor_bookings)
    for booking in tractor_bookings:
        provider = booking.tractor_owner.user
        booking.track_url = _build_google_maps_url(
            booking.location,
            provider.village,
            provider.mandal,
            provider.district,
    ...
    tool_bookings = list(tool_bookings)
    for booking in tool_bookings:
        provider = booking.tool_shop.user
        booking.track_url = _build_google_maps_url(
            booking.delivery_location if booking.home_delivery else None,
            provider.village,
            provider.mandal,
            provider.district,
            provider.state,
    ...
    filtered_groups = {
        'labor': bool(labor_bookings),
        'tractor': bool(tractor_bookings),
        'tool': bool(tool_bookings),
        'lease': bool(lease_requests),
        'shop': bool(shop_orders),
    }
    has_filtered_bookings = any(filtered_groups.values()) if service_filter == 'all' else filtered_groups.get(service_filter, False)
    ...
    context = {
        ...
        'has_bookings': has_filtered_bookings,
    }
...
def book_tool(request, tool_id):
    ...
    if request.method == 'POST':
        ...
        if not tool_prices:
            tool_prices = _extract_hourly_rates(tool_shop.tools_type)
...
def cart_view(request):
    ...
    order_status_filter = (request.GET.get('order_status') or 'all').strip().lower()
    active_tab = (request.GET.get('tab') or ('cart' if cart_data else 'tracking')).strip().lower()
    if active_tab not in {'cart', 'tracking'}:
        active_tab = 'cart' if cart_data else 'tracking'
    ...
    if order_status_filter != 'all':
        orders = orders.filter(status=order_status_filter.title())

    return render(request, 'kisan1/cart_orders.html', {
        'farmer': farmer,
        'orders': orders,
        'cart_data': cart_data,
